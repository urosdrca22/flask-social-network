...
def login():
    ...
    return (str(current_user_id), session['user_id'])

@app.route('/newpost', methods=('GET', 'POST'))
def new_post():
    current_user_id = session['user_id']
    if request.method == 'POST':
        payload = {'title': 'Testing microservice communication',
                   'content': 'Adding new post from user microservice with user_id #3',
